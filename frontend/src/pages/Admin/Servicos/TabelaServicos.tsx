...
interface TabelaServicosProps {
  dados?: any[]
  load: (novoValor: Servico) => void
  remove: (dado: Servico) => void
}

export default function TabelaServicos(props: TabelaServicosProps) {
  const { data } = useGncData()
  const [excluir, setExcluir] = useState<boolean>(false)
  const [dado, setDado] = useState<Servico>(initialServico)
...
function abrir(registro: Servico) {
  setExcluir(true)
  setDado(registro)
}
...
function dadosSearch() {
  ...
  props.dados?.map((registro: any) =>{
    ...
    if(registro.tipo?.toLowerCase().match(vl.toLowerCase())){
      dado.push({...registro})
    }
  })
  return dado
